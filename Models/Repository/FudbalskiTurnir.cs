...
namespace FudbalskiTurnir_FilipNikolic.Models.Repository
{
	public class FudbalskiTurnir : IFudbalskiTurnir
	{
		...
		public void DeleteRezultat(RezultatModel rezultat)
		{
		...
		public IEnumerable<RezultatModel> ReadAlLRezultat()
		{
			return _dbContext.Rezultati.ToList();
		}
		...
		{
			return (RezultatModel)_dbContext.Rezultati.Where(x => x.Id == id);
		}
		...
		public RezultatModel UpdateRezultat(RezultatModel rezultat, int id)
		{
			throw new NotImplementedException();
		}
		...
		public void UpdateTurnir(string ime)
		{
			throw new NotImplementedException();
		}
		...
		public List<RezultatModel> NapraviRezultate(TimModel[] timovi, int turnirId)
		{
			...
			if(_dbContext.Timovi.Any(x=>x.BrojPobeda !=0 || x.BrojIzgubljenih !=0 || x.BrojRemija !=0))
				{
				var timoviZaPodesavanjeParametara = _dbContext.Timovi.Where(x => x.TurnirId == turnirId);
        ...
        public void DodajIshode(RezultatModel rezultat)
		{
        ...
        public TimModel[] PoredjajTimovePoPoenima(TimModel[] timovi)
		{
			foreach(var t in timovi)
			{
				t.BrojOsvojenihBodovaNaTurniru = t.BrojPobeda * 3 + t.BrojRemija * 1;
			}
			var poredjani = timovi.OrderByDescending(x => x.BrojOsvojenihBodovaNaTurniru).ToArray();
			return poredjani;
		}
    }
}
